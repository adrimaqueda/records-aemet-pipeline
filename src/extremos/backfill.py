...
# datania solo mirroriza de forma continua a partir de 1975 (antes apenas tiene
# 1920-1922 sueltos). El histórico previo lo rellena `backfill_historico.py`.
MIN_BACKFILL_YEAR = 1975
...
def ingest_month(con: duckdb.DuckDBPyConnection, year: int, month: int) -> int:
    ...
    try:
        ...
        for fp in sorted(month_dir.glob("*.json")):
            with fp.open("rb") as f:
                rows.extend(normalize_observations(json.load(f)))
...
def main(argv: list[str] | None = None) -> None:
    ...
    api = HfApi()
    pairs = [
        (y, m) for y, m in list_year_months(api)
        if y >= args.from_year and (args.to_year is None or y <= args.to_year)
    ]
    log.info(
